...
public class UdpGetClientMacAddr {
    ...
    public UdpGetClientMacAddr(String strAddr) throws Exception {
        ...
        ds.setSoTimeout(CifsConstants.OUT_TIME_UNIT);
    }

    private final DatagramPacket send(final byte[] bytes) throws IOException {
        DatagramPacket dp = new DatagramPacket(bytes, bytes.length,
                InetAddress.getByName(sRemoteAddr), CifsConstants.REMOTE_PORT);
        ds.send(dp);
        return dp;
    }
    ...
    // Unit ID 6字节（48位
    private final LanNodeInfo getMacAddr(byte[] brevdata) throws Exception {
        ...
        return new LanNodeInfo(sRemoteAddr, new String(brevdata, 57, 18),
                new String(brevdata, 93, 18), sb.toString());
    }

    private final String getName(byte[] brevdata) throws Exception {
        // 获取计算机名
        int i = brevdata[56] * 18 + 56;
        String sAddr = "";
        StringBuffer sb = new StringBuffer(17);
    ...
    public final LanNodeInfo getRemoteMacAddr() throws Exception {
        byte[] bqcmd = GetQueryCmd();
        send(bqcmd);
        DatagramPacket dp = receive();
        LanNodeInfo info = getMacAddr(dp.getData());
        close();
